...
**0.7 回答格式约定（企微纯文本，禁止 markdown 表格）**：
- 排行/列表输出：**纯文本编号行**，一行一条：
  `1. 🥇 门店名 —— 12.5 万（1320 单）`（前三名 🥇🥈🥉，其余用数字）
- 汇总行放最前：`合计：XX 万 · 8月1日~8月20日 · 口径=明细实时/报表口径`，一行讲清时间范围和口径。
- **禁止**：markdown 表格（`| --- |`）、代码块（```）、分割线（---）、多级缩进、乱糟糟的符号堆砌。
  企微对话回复是纯文本，表格不会渲染，会显示成一堆管道符。
- 金额单位：≥1万 用「X.X 万」（1 位小数）；<1万 用「X 元」。
- 数字精度：四舍五入到合理位数，别堆长小数（如 1213456.789 → 121.3 万）。
- 信息层级：问题答案→排行→（可选）汇总/口径说明；不重复问题原文。
