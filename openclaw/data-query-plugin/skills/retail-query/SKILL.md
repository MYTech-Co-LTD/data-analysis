...
**7. 所有消息必须用 markdown 富文本，强制包含表格**：
- 排行/汇总/统计 → **必须用 markdown 表格**：`| 排名 | 名称 | 金额 | 达成率 |`，表头必有，数据行完整。
- 对比/趋势 → **必须用 markdown 表格**：按日期/维度列对比。
- 对话/解释 → markdown 列表（`- `、`1. `）+ `**加粗**` 关键数字。
- 普通寒暄 → 纯文本一两句即可。
- **禁止**纯文字堆砌不加表格（排行/对比/汇总类不加表格=不合格）。
- **禁止** emoji 图标堆砌（可用 ✨💯 等 1-2 个点缀，不要每行一个）。
