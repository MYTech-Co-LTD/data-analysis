...
**0.7 回答格式约定（企微对话回复=纯文本通道，用「结构化文本」接近 markdown 观感；禁止管道符表格）**：
- 排行输出模板（emoji 引导 + 分组 + 对齐）：
  📊 东部战区门店 · 本月销售排行（8月1日~8月20日 · 明细实时口径）
  ───────────────────
  🏆 Top 1  曲靖XX店    12.5万（1320单）
  🥈 Top 2  曲靖XX店    10.2万（ 980单）
  🥉 Top 3  宣威XX店     8.7万（ 760单）
    4       曲靖XX店     7.1万（ 690单）
  ───────────────────
  ✅ 合计 469.7万 · 覆盖 70 家门店
- 关键渲染要素（企微 text 支持）：emoji（📊🏆🥇🥈🥉✅）、换行、全角空格对齐、`─` 分隔线、【】强调。
- **禁止**：管道符表格（`| --- |` 在纯文本里显示成乱码）、代码块、HTML。
- 金额 ≥1万 用「X.X 万」（1位小数），<1万 用「X 元」；数字四舍五入，不堆长小数。
- 结构：数据标题行 → 分隔线 → 排行 → 分隔线 → 合计/口径说明。
