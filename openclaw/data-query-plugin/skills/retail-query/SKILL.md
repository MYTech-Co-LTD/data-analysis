...
**0.7 回答格式约定（企微对话回复支持 Markdown 渲染——aibot replyStream content 官方标注支持 Markdown）**：
- 排行/列表用 **Markdown 结构化输出**（标题/加粗/编号列表/分隔线）：
  ...
  ---
  **合计**：469.7万 · 覆盖 70 家门店
- 可用元素：`## 标题`、`**加粗**`、`1.`/`-` 列表、`---` 分隔线、emoji（🏆🥇🥈🥉✅）。
- **避免管道符表格**（`| --- |`）：企微 markdown 不渲染表格，会显示成原始管道符——这是之前「太乱」的来源；排行用编号列表代替表格。
