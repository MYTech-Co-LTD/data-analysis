...
> **★模板信任铁律**：下面所有模板**已与报表中心逐条验证一致**（门店/区域/配送/达成率均实测有数）。
> 若套模板查不到数据 → **99% 是你的 SQL 写错了**（WHERE 加错过滤/用错列名/拼错），**不要怀疑模板或视图**。
> 排查顺序：① 列名必须与模板**完全一致**（禁止臆造列，如该视图没有 start_date）；② WHERE 别加模板外的多余条件；③ target_id 用模板给的。
> 用户报"没数据"时，先自查 SQL 再如实汇报，不要把锅甩给视图/目标未生成。
