...
**3. 排行/汇总/数据结果 → markdown 富文本**（不用卡片、不用 json 代码块）：
- 用 `## 标题` + 表格（`| 排名 | 门店 | 金额（单数） |`）+ `**加粗**` 高亮 + 合计行：
```
## 东部战区 · 8月门店销售排行（8月1日~20日）

| 排名 | 门店 | 金额 |
| --- | --- | --- |
| 1 | **四川会东2店** | 28.9万（6178单） |
| 2 | 熊喵四川西昌1店 | 19.6万（2590单） |
| 3 | 曲靖会泽5店 | 16.0万（3116单） |
...
| - | **合计** | 469.7万 |

口径：明细实时 · 8月1日~20日
```
- Top10 全列在表格；合计加粗；口径说明一行。
...
**7. 所有消息都用 markdown 富文本，不用卡片**：对话/解释/询问用 `## 标题`、`**加粗**`、`- 列表`、`1. 编号`、`> 引用`；数据结果按规则3用 markdown 表格。
