...
-- M-1 角色码统一（spec 2026-08-15 平台级权限改造 §4.1 / BLOCKER 双键债）：
--   data_permissions role 行 subject_id 从 role_id::text 统一为 roles.code。
-- 幂等：约束 DO 块容错、快照 CREATE TABLE IF NOT EXISTS（仅首跑落数据）、UPDATE 重跑 no-op、函数 CREATE OR REPLACE。
-- 顺序铁律：快照必须在键切换【前】落（用旧函数×旧键）；函数改写必须在键切换【后】（code 匹配×新键）。
-- 门禁：迁移后跑 scripts/m1_gate.sql（逐用户 diff=0 才 exit 0）。
-- 反向：database/rollback/168_role_code_unification_reverse.sql（不在 migrations/ 目录——migrate.sh 全量重跑会把键改回去）。
BEGIN;

-- ① 命名空间约束（RT-14）：code 不允许纯数字——否则某角色的 code 可能与其它角色的
--    role_id::text 同形，code 键与遗留数字键错映射。072 已有列级 UNIQUE（roles_code_key），
--    此处显式补命名约束 + 冗余唯一约束（DO 块容错重跑）。
--    （42P07 duplicate_table：唯一约束底层索引同名已存在；42710 duplicate_object：约束同名已存在）
DO $$ BEGIN
...
-- ② 迁移前快照（仅首跑有数据；IF NOT EXISTS 重跑跳过）——门禁比对基准。
...
COMMENT ON TABLE perm_migration_snapshot IS 'M-1 角色码统一迁移前权限快照（168 首跑落；scripts/m1_gate.sql 逐用户 diff=0 门禁基准；迁移稳定后可清理）';

-- ③ 键切换：role 行 subject_id → roles.code（重跑 no-op）。
UPDATE data_permissions dp SET subject_id = r.code
  FROM roles r
  WHERE dp.subject_type = 'role' AND dp.subject_id = r.id::text;

-- ④ get_user_perms role 行匹配改走 code（167 原版按 role_id::text 匹配，键切换后须同步，
--    否则 role 基底失配 → 空基底兜底 ["*"] = 权限静默放大）。仅改角色行匹配两处
--    （dp.subject_id = v_role_code / r.code = dp.subject_id），其余与 167 逐字一致。
...
BEGIN
  ...
  --    168：role 行匹配由 role_id::text 改为 code（v_role_code 仅在角色 active 时非 NULL，
  --    与 EXISTS 双保险保留 F3「停用角色剥离基底」语义）
  WITH rows AS (
...
END;
$$;
COMMENT ON FUNCTION get_user_perms(VARCHAR) IS '权限合成 RPC：角色∪部门基底叠加，个人 override 按字段覆盖（NULL=不覆盖）；返回四维+角色 UI 字段。168 起 role 行 subject_id=roles.code';
