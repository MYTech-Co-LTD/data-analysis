...
-- ---- compute 定义：item_outbound 加 branch_num（157 形态 + all.parquet；delivery=response_branch_num / wholesale=branch_num∪收货方门店） ----
UPDATE report_definitions SET
  sql_template = $SQL$
WITH delivery AS (
  SELECT regexp_extract(d.filename,'transfer_detail/([0-9]+)/',1) AS system_book_code,
    substr(d.order_time,1,4)||substr(d.order_time,6,2)||substr(d.order_time,9,2) AS biz_date_raw,
    d.response_branch_num AS branch_num, d.item_num,
...
wholesale AS (
  SELECT COALESCE(db.system_book_code, regexp_extract(d.filename,'wholesale_detail/([0-9]+)/',1)) AS system_book_code,
    ...
    COALESCE(db.branch_num, d.branch_num, '') AS branch_num, d.item_num,
    MAX(d.pos_item_code) AS pos_item_code,
...
FROM delivery d FULL OUTER JOIN wholesale w
  ON d.system_book_code=w.system_book_code AND d.biz_date_raw=w.biz_date_raw
   AND d.branch_num=w.branch_num AND d.item_num=w.item_num
$SQL$,
