-- 187_scope_match_v2_cache.sql
-- scope_match_v2 会话级缓存正式化（2026-08-17 生产 503 修复链性能收尾）。
-- 依赖：186（复合门店键尾段前导零归一）。本迁移将 186 函数体替换为生产实测缓存版
--   （pg_get_functiondef 原样对齐，防止下次部署覆盖后漂移）。签名 (p_dim text, p_col text) 不变，
--   视图依赖不破，migrate.sh 重跑幂等（CREATE OR REPLACE）。
--
-- 相对 186 的三处增量（2026-08-17 生产手动 CREATE OR REPLACE 优化，原不在仓库，已核对无正确性缺陷）：
--   ① 非法维度防御：v_allowed 白名单，未知 p_dim 直接 deny（防手误传错维度名全放）
--   ② 会话级缓存：claims 文本变化时用 jsonb_object_agg 一次算全 3 维归一化数组，存
--      app.sm2_last / app.sm2_map（会话作用域 GUC）；同请求多行 RLS 判定 O(1) `?` 包含替代 FOREACH
--   ③ x_grants 侧不走缓存：例外集量级小，每行内联归一，避免缓存语义与 claims 侧耦合
--
-- 语义保持核对（与 186 逐条对齐）：
--   · data_scope 缺失 / 段缺失 / 非数组 / 并集空（B1）→ deny（fail-close）
--   · 通配 '*' 任一侧 → 放行；裸值（无 '-'）与复合值互不匹配（branch_num 跨账套不唯一，不放宽）
--   · 归一化作用于 data_scope 与 x_grants 两侧 + p_col（双侧对称，方向无关）
BEGIN;
...
COMMENT ON FUNCTION scope_match_v2 IS
  'W6 终版+187 缓存化：data_scope.<dim> ∪ x_grants.<dim>（x_grants 独立 GUC）；并集空=deny（B1）、通配任一侧放行；无 data_scope 段=deny（S4 已关）；复合门店键尾段前导零归一（186）；会话级缓存 app.sm2_last/map（187，O(1) 包含查找）';

-- 回滚口径：CREATE OR REPLACE 回 186 版函数体（删 v_allowed/v_last/v_map/v_dim_norm/v_gdim_norm
--   声明、非法维度防御、缓存三段与 ? 包含查找，恢复 FOREACH 循环，GRANT/COMMENT 不变）。

DO $$ BEGIN RAISE NOTICE 'Migration 187: scope_match_v2 会话级缓存正式化（生产实测版对齐，防部署漂移）'; END $$;
