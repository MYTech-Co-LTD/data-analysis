DROP VIEW IF EXISTS report_region_breakdown_gen CASCADE;
...
leaf_act_2 AS (
  SELECT tgt.target_id, s.system_book_code, s.branch_num,
    ...
    SUM(s.wholesale_amount) FILTER (WHERE s.biz_date = tgt.latest_day) AS daily_delivery
...
leaf_tgt AS (
  ...
  WHERE t.breakdown_level = 'store' AND t.branch_num <> 'ALL' AND (t.branch_num = 'ALL' OR ('*' = ANY((SELECT scope_brand_keys())::text[]) OR t.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR t.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (t.system_book_code || '-' || t.branch_num) = ANY((SELECT scope_branch_keys())::text[]))) AND is_assessed_war_zone(db.first_level_region)
...
leaf_rows AS (
  ...
  COALESCE(a2.wholesale_pp_amount, 0) AS wholesale_pp_amount,
  COALESCE(a0.daily_sale, 0) AS daily_sale,
  COALESCE(a1.daily_delivery, 0) + COALESCE(a2.daily_delivery, 0) AS daily_delivery,
  COALESCE(a3.sale_target, 0) AS sale_target,
  COALESCE(a3.delivery_target, 0) AS delivery_target,
  tgt.total_days,
  tgt.days_elapsed
...
region_tgt_pre AS (
  ...
  FROM targets t
  JOIN target_metric_values tmv ON tmv.target_id = t.id AND (metric_code='sale' OR metric_code='delivery')
  WHERE t.breakdown_level = 'war_zone' AND (t.branch_num = 'ALL' OR ('*' = ANY((SELECT scope_brand_keys())::text[]) OR t.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR t.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (t.system_book_code || '-' || t.branch_num) = ANY((SELECT scope_branch_keys())::text[]))) AND is_assessed_war_zone(t.war_zone)
...
sub_region_tgt_pre AS (
  ...
  FROM targets t
  JOIN target_metric_values tmv ON tmv.target_id = t.id AND (metric_code='sale' OR metric_code='delivery')
  WHERE t.breakdown_level = 'region_l2' AND (t.branch_num = 'ALL' OR ('*' = ANY((SELECT scope_brand_keys())::text[]) OR t.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR t.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (t.system_book_code || '-' || t.branch_num) = ANY((SELECT scope_branch_keys())::text[]))) AND is_assessed_war_zone(t.war_zone)
...
SELECT
  ...
  COALESCE((COALESCE(a.delivery_amount, 0) + COALESCE(a.wholesale_pp_amount, 0)), 0) AS delivery_actual,
  ...
  round(((COALESCE(t.sale_target, 0) - COALESCE(a.sale_amount, 0)) / greatest((a.total_days - a.days_elapsed), 1)), 2) AS remaining_daily_sale_target,
  round(((COALESCE(t.delivery_target, 0) - ((COALESCE(a.delivery_amount, 0) + COALESCE(a.wholesale_pp_amount, 0)))) / greatest((a.total_days - a.days_elapsed), 1)), 2) AS remaining_daily_delivery_target
...
SELECT
  ...
  COALESCE(a.daily_delivery, 0) AS daily_delivery,
  round(((COALESCE(a.sale_target, 0) - COALESCE(a.sale_amount, 0)) / greatest((a.total_days - a.days_elapsed), 1)), 2) AS remaining_daily_sale_target,
  round(((COALESCE(a.delivery_target, 0) - ((COALESCE(a.delivery_amount, 0) + COALESCE(a.wholesale_pp_amount, 0)))) / greatest((a.total_days - a.days_elapsed), 1)), 2) AS remaining_daily_delivery_target
FROM leaf_rows a;
