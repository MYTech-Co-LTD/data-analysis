...
FROM (
  SELECT 'wholesale_ext_amount' AS metric,
    ...
    COALESCE((SELECT COALESCE(SUM(s.wholesale_money), 0) FROM report_daily_wholesale s JOIN targets t ON s.biz_date BETWEEN t.start_date AND LEAST(current_date, t.end_date) AND t.target_level = 'total' AND t.status IN ('active','closed') WHERE s.system_book_code = '3120'), 0) AS ref_sum
) t;
