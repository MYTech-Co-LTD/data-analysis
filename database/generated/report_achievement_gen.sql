...
WITH tgt AS (
  ...
  FROM targets t WHERE t.target_level = 'total' AND (t.branch_num = 'ALL' OR ('*' = ANY((SELECT scope_brand_keys())::text[]) OR t.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR t.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (t.system_book_code || '-' || t.branch_num) = ANY((SELECT scope_branch_keys())::text[])))
),
...
LEFT JOIN report_daily_sales r
  ...
  AND ('*' = ANY((SELECT scope_brand_keys())::text[]) OR r.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR r.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (r.system_book_code || '-' || r.branch_num) = ANY((SELECT scope_branch_keys())::text[]))
...
delivery AS MATERIALIZED (
  ...
  COALESCE(SUM(d.out_money), 0) + COALESCE((
      ...
      WHERE w.system_book_code = '64188' AND w.biz_date BETWEEN t.start_date AND t.end_date
        ...
        AND ('*' = ANY((SELECT scope_brand_keys())::text[]) OR w.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR w.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (w.system_book_code || '-' || w.branch_num) = ANY((SELECT scope_branch_keys())::text[]))
    ), 0) AS actual_value,
...
LEFT JOIN report_daily_delivery d
  ...
  AND ('*' = ANY((SELECT scope_brand_keys())::text[]) OR d.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR d.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (d.system_book_code || '-' || d.branch_num) = ANY((SELECT scope_branch_keys())::text[]))
WHERE t.target_level = 'total'
GROUP BY t.id
),
...
LEFT JOIN report_daily_delivery d FULL OUTER JOIN report_daily_wholesale w
  ...
  AND ('*' = ANY((SELECT scope_brand_keys())::text[]) OR COALESCE(d.system_book_code, w.system_book_code) = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR COALESCE(d.branch_num, w.branch_num)::text = ANY((SELECT scope_branch_keys())::text[]) OR (COALESCE(d.system_book_code, w.system_book_code) || '-' || COALESCE(d.branch_num, w.branch_num)) = ANY((SELECT scope_branch_keys())::text[]))
WHERE t.target_level = 'total'
GROUP BY t.id
),
...
scoped_tgt AS MATERIALIZED (
  ...
  WHERE t.breakdown_level = 'store' AND t.branch_num <> 'ALL' AND ('*' = ANY((SELECT scope_brand_keys())::text[]) OR t.system_book_code = ANY((SELECT scope_brand_keys())::text[])) AND ('*' = ANY((SELECT scope_branch_keys())::text[]) OR t.branch_num::text = ANY((SELECT scope_branch_keys())::text[]) OR (t.system_book_code || '-' || t.branch_num) = ANY((SELECT scope_branch_keys())::text[]))
