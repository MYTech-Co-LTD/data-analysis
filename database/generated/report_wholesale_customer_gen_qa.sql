...
FROM (
  SELECT 'wholesale_amount' AS metric,
    ...
    COALESCE((SELECT COALESCE(SUM(s.wholesale_amount), 0) FROM report_daily_wholesale_customer s JOIN targets t ON s.biz_date BETWEEN t.start_date AND t.end_date AND t.target_level = 'total' AND t.status = 'active'), 0) AS ref_sum
) t;
