...
SELECT tgt.target_id,
  ...
  CASE WHEN can_cost_visible() THEN COALESCE((COALESCE(cte1.delivery_profit, 0) + COALESCE(cte2.wholesale_pp_profit, 0)), 0) END AS delivery_profit,
  CASE WHEN can_cost_visible() THEN round((((COALESCE(cte1.delivery_profit, 0) + COALESCE(cte2.wholesale_pp_profit, 0))) / NULLIF(((COALESCE(cte1.delivery_amount, 0) + COALESCE(cte2.wholesale_pp_amount, 0))), 0)), 4) END AS delivery_margin
FROM dim_brand b
CROSS JOIN tgt
...
UNION ALL
SELECT tgt.target_id, '合计' AS system_book_code, NULL AS brand_name, SUM(brand_rows.sale_amount) AS sale_amount, SUM(brand_rows.sale_target) AS sale_target, round(COALESCE(SUM(brand_rows.sale_amount), 0) / NULLIF(COALESCE(SUM(brand_rows.sale_target), 0), 0), 4) AS sale_rate, SUM(brand_rows.delivery_amount) AS delivery_amount, CASE WHEN can_cost_visible() THEN SUM(brand_rows.delivery_profit) END AS delivery_profit, CASE WHEN can_cost_visible() THEN round(COALESCE(SUM(brand_rows.delivery_profit), 0) / NULLIF(COALESCE(SUM(brand_rows.delivery_amount), 0), 0), 4) END AS delivery_margin
