...
- **横幅渲染（2026-08-20 方案 C，推送时数据驱动）**：template_card `news_notice` 的 `card_image` 由静态占位图（`web/public/push/daily-report-banner.png`）升级为推送时渲染的动态横幅——数据写进签名 URL，企微抓图时 GET 路由现场渲染，**无保存时快照**。
  - **数据流**：推送触发 → 引擎解析变量（数值变量已代签 JWT 解析出原始值，**B2：值嵌入 URL，不二次查库**）→ 渲染 preset 时把 `card_image.url` 注入签名 URL `https://data.shanhaiyiguo.com/api/push/banner?d=YYYY-MM-DD&t=<target_id>&sale=<达成>&rate=<达成率>&sig=HMAC-SHA256` → 企微无会话抓图 → GET 路由验签 → sharp 渲染 SVG→PNG（1080×480，aspect 2.25）→ 返回 + 进程内缓存（键=(date,target)，TTL+上限）。
  - **鉴权**：企微抓图无 cookie 会话（WeCom 侧拉取），故 URL 带 HMAC-SHA256 签名（JWT_SECRET 派生 secret），防遍历/防伪造。banner 数据 = 卡片 `vertical_content_list` 已展示数据，**无新增暴露面**。
  - **中文渲染**：SVG `@font-face { src: url(data:application/font-otf;charset=utf-8;base64,…) }` data URI 内嵌 Noto Sans SC 子集（OFL 协议，商用合规）——**生产 alpine 容器无 CJK 字体也不依赖系统字体**，SVG 自包含，dev/prod 像素一致，零 Dockerfile 改动（已验证 sharp 可渲染中文字体）。
  - **依赖**：sharp 从 next/image 传递依赖提升为 web 显式直接依赖（0.34.5）。缓存用进程内 Map（TTL+条目上限），避免每日重复渲染。
  - **影响面**：只改 web/（引擎注入 + 新 GET 路由 `web/app/api/push/banner/route.ts` + 字体子集资产 + package.json）——无新表/无迁移/无 function/无 nginx 改动（`/api/push` 已由网关代理到 web:3000）。GHA 完整部署。
