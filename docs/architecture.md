...
- **定时报表推送目标选择（2026-09-10 定稿）**：`push_configs.target_mode` 两态——`fixed` 锁定单目标（`target_id`）；`follow` = **扇出全部「进行中」目标**（`report_achievement_gen` 中 `target_level='total'` + `status='active'` + 今天落 `[start_date,end_date]`，按 target_id 去重），`scheduled-reports` job 对每个目标各以 `fixed`+`target_id` 调一次 run_push → **一个目标一条卡片**。修正前 follow 为单目标 tie-break（`order=start_date.desc,end_date.asc` 取「粒度最细」一个），重叠周期下只推一个目标（如 9月目标遮蔽中秋目标），2026-09-10 改为扇出（无进行中目标仍走 target-guard 跳过 + owner 提醒；部分失败写 last_run_date 防重复、全失败不写以便下轮重试）。
...
**目标窗口机制（2026-09-02 修订——千人千面；原 2026-08-03「统一 active + closed 读快照」废弃）**：两层模型。
- **KPI 层 `report_achievement_gen`**：closed total 目标——全量用户（`branch_scope_limited()`=false）读 `target_snapshots` 定格快照（snapGuard `t.status='closed' AND NOT branch_scope_limited()`，2026-08-19 方案 B 定稿）；受限用户走 live 重算（tgt CTE `permFilterTarget` 收缩目标行 + 194 `scoped_tgt` 分母收缩）。快照定格由 `close_target` 完成——**迁移 210 起函数体内 `set_config` 注入全量 claims**（`{"data_scope":{"brands":["*"],"branch_nums":["*"]},"fields":{"cost":true}}`），以服务级权威身份固化：此前 SECURITY DEFINER 服务上下文无 claims → `scope_brand_keys()` fail-close 返 ∅ → 快照全零空壳（实例 823）。迁移 210 自带 `GRANT EXECUTE`，823 修复为一次性 SQL（改回 active → 重调 `close_target`，不入迁移）。
- **下钻 8 视图（brand/region/category/item/supply/wholesale_daily + 批发客户两视图）**：`target_status: ['active','closed']` 统一实时重算 + 行级 scope 过滤（`permFilterFact`/`claim_match_or_star`）→ closed 目标下钻千人千面继承；`target_snapshot_breakdowns` JSONB 降级为审计存档，前端 getter 已删 closed 快照分支。批发客户两视图无 qa 断言覆盖，closed 收编无对账影响。
- **一致性铁律**：视图 `target_status` 与 `qa-checks.json` 断言 ref_sql 的 status 子句必须一致（否则 C2 对账误报）；**迁移 210 必须与前端去快照 getter 同批上线**——210 单独落地则受限用户经旧前端 getter 读 `target_snapshot_breakdowns` 无行级过滤 → 全量快照泄漏。
**达成视图生成器（2026-08-03 语义层改造）**：`report_achievement_gen`（target×metric 矩阵：目标列表 + KPI）由独立生成器 `src/generators/achievement.ts` + 配置 `src/achievement-config.ts` 产出——替代手写 `report_achievement_v`。各指标 actual 计算是**配置数据**（SQL 片段引用 t=targets，sale/delivery/outbound 口径照迁移 118 total 级），生成器只做结构化组装（tgt 窗口 + metric CASE + closed 读 snapshot + 达成率/月进度），无业务字面量。前端只消费 `target_level='total'` 行。**权限过滤（2026-08-05 补）**：接入 perm 注入--actual CTE 经 config `factAlias`/占位符注入 `permFilterFact(alias, skipBranch)`（delivery 内嵌 wholesale_customer `w` 用 `skipBranch=true`），FULL JOIN（outbound `d` FULL JOIN `w`）用新增 `permFilterFullJoin`（COALESCE 取非空侧过滤），tgt CTE 加 `permFilterTarget`；closed 目标仅全量用户读 `target_snapshots` 快照（受限用户 live 重算，见上「目标窗口机制」2026-09-02 修订；定格经迁移 210 注入全量 claims，保证快照本身是全量口径）。**C2 对账接入（2026-08-05 补）**：achievement_gen 接入 `_qa` 产出——`qa-checks.json` 3 条断言（sale/delivery/outbound_amt，active 目标口径），长表 SUM `actual_value` WHERE `metric_code`+`status='active'`（closed snapshot 行排除），ref_sql 独立重算 active total 周期明细，gen 后即时断言（diff>0.01 即 exit 1），与其它视图 C2 覆盖对齐。
