...
- 一句话：**先角色、再权限、挂人靠薄同步自动、管理台单独授、范围随组织架构自动成立**。
...
### 6. 数据范围确认（门店=组挂载、品牌/品类/成本=角色档位）
- 门店 = 部门组挂载（企微部门树自动同步）→ groups claim → `data_scope.branch_nums` → RLS 行过滤；一般自动成立，确认挂对组即可；异常补挂 Casdoor → 组织架构 → 目标组 → 编辑成员
