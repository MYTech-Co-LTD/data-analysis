...
- **门店范围要显式配（2026-08-18 起，废除组织架构推导）**：在该用户的 Casdoor permission Resources 加 `范围|全店` / `范围|战区包名` / `范围|branch_number` / `范围|门店中文名`；**没配 = 空集 deny**（看不到任何门店数据，fail-close）。不再从部门组自动推导。
  临时放开/收窄 = 直接改 `范围|X` 资源（挂上到期摘除）——例外体系已废除。
...
## 六、临时放开 / 收窄（2026-08-18 起：直接改范围资源，例外体系已废除）

特殊场景（临时看某门店、临时收窄）**不走例外（已废除）**，直接改 `范围|X` 资源：

- **临时放开**：在该用户 Casdoor permission Resources 加 `范围|XX`（如临时看某店）→ 到期手动摘除
- **临时收窄**：摘掉多余的 `范围|XX`（如战区经理临时只看一区）→ 事后挂回
