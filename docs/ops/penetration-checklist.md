...
- [x] **T4**（2026-08-17 真机）：provisioning JIT 真机（TestLiZhi001 04:30 tick 建户链路）✅；disable 真机（05:00 tick，见 T6）✅；assign_role 通道**已真机启用**（2026-08-17 角色层落地：5 角色建 + 45/45 active 用户 Role.Users 挂载 + assignRoles 改造 update-role 全量写 + outbox 46 条积压清理归零，见 docs/ops/casdoor-role-permission-mechanism.md §4）。**发现并修复三处断链**（PR#25/#36）：provision 假成功（signupApplication 指向不存在 app + body 判红缺失）、disableUser 从未真正禁用（update-user 形态错）、assignRoles 外壳 bug + **add-role-for-user 404（本版本无此 API，改 update-role 全量 Users）**。outbox 注入失败重放机制真机验证（disable 失败入队路径）。drift 告警待后续阶段。
...
- [x] **角色层落地**（2026-08-17 真机，方案 B）：Casdoor 建 5 角色（boss 8/zone_manager 0/finance 1/manager 36/buyer 0，Role.Users 挂载 45/45 active 用户）→ 建 5 permission（role-boss/role-zone_manager/role-finance 各 20 资源含 cost，role-manager/role-buyer 各 19 资源）→ addPolicies 生成 98 条角色 p 策略 → 旧 basic/full 停用（isEnabled=false）→ **全量对账 44/45 全等、0 差异**（去重后集合；唯一例外 YiBeiMeiShi. Casdoor 无户待 JIT 补建）→ 权限语义与迁移前完全一致 ✅。配套修复：claims.js permissions 去重（get-all-objects 并集路径重复）；YiBeiMeiShi. casdoor_synced_at 置 NULL 待 JIT；outbox 46 条积压清理归零。