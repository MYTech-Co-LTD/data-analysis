...
export default function CapabilitiesPage() {
  ...
  return (
    <div className="font-sans">
      ...
      {data && (data.cycleCheck.length > 0 || data.synced.unknown || (!data.synced.ok) || !data.reconcile) && (
        <div className="mb-4 flex flex-col gap-1.5">
          ...
          {!data.synced.unknown && !data.synced.ok && (
            <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
              <CircleAlert size={15} className="shrink-0 mt-0.5" />
              <span>有 {data.synced.missing.length} 个能力点注册 Casdoor 失败（见红区 C-sync-failed）——失败能力永不可配，不会静默。</span>
            </div>
          )}
          {!data.reconcile && (
            <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
      ...
      {/* catalog 全量（按组分组：key/组/标签/敏感/来源/synced） */}
      <section>
        <h2 className="text-sm font-semibold text-slate-800 mb-2">能力点清单（{data?.entries.length ?? 0}）</h2>
        <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
          <table className="w-full text-sm border-collapse tabular-nums">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-200 bg-slate-50">
