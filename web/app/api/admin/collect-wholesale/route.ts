// web/app/api/admin/collect-wholesale/route.ts
// 批发销售明细采集手动触发入口（照 collect-delivery route）
import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@insforge/sdk';
import { collectWholesaleOnce, getTodayChina, getYesterdayChina, LEMENG_SECRET_KEY, type WholesaleCollectResult } from '@/lib/collect-wholesale';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_VERIFY_RETRIES; attempt++) {
      lastResult = await collectWholesaleOnce(authToken, branchNumsStr, dates.from, dates.to, limit);
