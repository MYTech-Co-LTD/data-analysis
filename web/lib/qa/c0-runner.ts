...
export interface C0RunnerOpts {
   ...
   *  仅完结日（每日 09:15 回溯窗口）传 true；当天窗口（coarseToday=true）必须关——当天源持续增长，
   *  每次 full 重采一个移动目标无意义且反复打乐檬 API（2026-08-07 一次 64188 run 内 6 次 transform）。 */
  autoBackfill?: boolean;
  /** 当天粗粒度健康检查（采集后 QA 传 true）：跑 coarse 判定（lib≥50% api 即 pass）+ 强制不 autoBackfill。
   *  精确对账与补采交给每日 09:15 完结日窗口（coarseToday 不传）。 */
  coarseToday?: boolean;
}
...
export async function runC0Checks(opts: C0RunnerOpts): Promise<CheckResult[]> {
  const { client, duck, runId, trigger, checks, window: win, autoBackfill, coarseToday } = opts;
    ...
    try {
      ...
      for (const task of tasks) {
        ...
        for (const dayIso of isoDaysBetween(fromIso, toIso)) {
          ...
          let r = await runC0(src, dayIso, counts.apiCount, counts.libCount, { apiFailed: counts.apiFailed, libMissing: counts.libMissing, coarse: opts.coarseToday });
          ...
          // coarseToday（当天窗口）强制不补采——当天源持续增长，重采移动目标无意义且耗 API。
          if (!opts.coarseToday && autoBackfill && r.status === 'fail' && (r.detail as any[])?.[0]?.verdict === 'missing') {
            let retries = 0;
