    ...
    try {
      ...
      for (const [bizday, recs] of Object.entries(byBizday)) {
        const duckRes = await fetch(`${DUCKDB_URL}${endpoint}`, {
          ...
          body: JSON.stringify({ records: recs, config: { date: bizday, source: 'lemeng', partition_by: [], dedupe_key: ['pos_order_num', 'item_num', 'client_code'], required_fields: ['pos_order_num', 'item_num', 'branch_num'], output_format: 'parquet', compression: 'zstd', base_path: `lemeng/wholesale_detail/${companyId}/${bizday}` } })
        });
