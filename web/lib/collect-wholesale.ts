...
// ⚠️ 乐檬返 code 是字符串 "0"/"-1"——必须 String 比较，严格 `=== 0` 恒 false（实测 2026-08-03）
...
const NATURAL_KEY = ['pos_order_num', 'item_num', 'client_code', 'wholesale_num', 'lot_number'] as const;
...
async function verifyDedup(bizday: string, companyId: string): Promise<{ total: number; distinct: number } | null> {
  const path = `s3://lemeng-datasource/lemeng/wholesale_detail/${companyId}/${bizday}/all.parquet`;
...
export async function collectWholesaleOnce(
  ...
  while (offset < result.apiTotal) {
    ...
    let bodyStr = buildBody(dateFrom, dateTo, offset, limit);
    let pr = await callLemengApi(ENDPOINT_DETAIL, authToken, bodyStr, branchNumsStr);
    let pageAttempt = 0;
    while ((!pr.ok || pr.data?.code != 0) && pageAttempt < 2) {
      pageAttempt++;
      result.pageFailures++;
      console.warn(`[collect-wholesale] offset ${offset} error (${pr.error || pr.data?.msg}), retry ${pageAttempt}/2...`);
      await sleep(randDelay());
      bodyStr = buildBody(dateFrom, dateTo, offset, limit);
      pr = await callLemengApi(ENDPOINT_DETAIL, authToken, bodyStr, branchNumsStr);
    }
    if (!pr.ok || pr.data?.code != 0) {
      result.pageFailures++;
      console.error(`[collect-wholesale] offset ${offset} failed after ${pageAttempt + 1} attempts: ${pr.error || pr.data?.msg}`);
      consecutiveErrors++;
  ...
  if (result.records.length > 0) {
    try {
      ...
      // 按数据实际日期(audit_time→YYYYMMDD)分区写，避免回溯多天时全月数据写到 dtFrom 目录致 /compute 翻倍
      const byBizday: Record<string, any[]> = {};
      for (const r of flat) {
        const raw = String(r.audit_time || '').slice(0, 10).replace(/-/g, '') || dateStr;
        (byBizday[raw] ||= []).push(r);
      }
      let lastPath = '';
      for (const [bizday, recs] of Object.entries(byBizday)) {
        const duckRes = await fetch(`${DUCKDB_URL}${endpoint}`, {
          ...
          body: JSON.stringify({ records: recs, config: { date: bizday, source: 'lemeng', partition_by: [], dedupe_key: [...NATURAL_KEY], required_fields: ['pos_order_num', 'item_num', 'branch_num'], output_format: 'parquet', compression: 'zstd', base_path: `lemeng/wholesale_detail/${companyId}/${bizday}` } })
        });
        ...
        if (dj.invalid_records > 0 || dj.duplicates_removed > 0) console.warn(`[collect-wholesale] ${bizday} quality: ${dj.invalid_records} invalid, ${dj.duplicates_removed} dup`);
      }
      ...
      for (const bizday of Object.keys(byBizday)) {
        ...
        if (v && v.total > v.distinct) {
          ...
          console.warn(`[collect-wholesale] ${bizday} 去重失效: total=${v.total} distinct=${v.distinct}（重复 ${v.total - v.distinct}）`);
        }
      }
