    ...
    try {
      ...
      for (const [bizday, recs] of Object.entries(byBizday)) {
        const duckRes = await fetch(`${DUCKDB_URL}${endpoint}`, {
          ...
          body: JSON.stringify({ records: recs, config: { date: bizday, source: 'lemeng', partition_by: [], dedupe_key: ['pos_order_num', 'item_num', 'response_branch_num'], required_fields: ['pos_order_num', 'item_num', 'response_branch_num'], output_format: 'parquet', compression: 'zstd', base_path: `lemeng/transfer_detail/${companyId}/${bizday}` } })
        });
