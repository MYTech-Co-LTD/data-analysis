...
// F2 加固（安全终检 BLOCKER）：在「cookie 非空」之上，增加 access_token 验签
// （JWT_SECRET / HS256）+ 过期校验 + sub 绑定：
...
// P0a（plan Task 3）：admin 判定从硬编码 userid 白名单收敛为
// checkFeaturePerm(uid, 'data-analysis:admin')（token claims 优先 + BREAKGLASS_ADMINS env 兜底）。
...
export async function requireAdmin(req: NextRequest): Promise<NextResponse | null> {
  ...
  // token claims 透传给功能门禁（permissions 数组，U2 后由 callback 平铺写入）
  const permissions = Array.isArray(payload.permissions)
    ? payload.permissions.filter((p): p is string => typeof p === 'string')
    : undefined;
  if (!(await checkFeaturePerm(uid, 'data-analysis:admin', { permissions }))) {
    return NextResponse.json({ ok: false, error: 'admin_required' }, { status: 403 });
  }
  return null;
}
